...
export const ModeratorDashboard: React.FC = () => {
  ...
  const load = async (nextTab = tab) => {
    ...
    setError('');
    try {
      setItems(await request(nextTab));
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    request(tab)
      .then(setItems)
      .catch((e: any) => setError(e.message || 'Không thể tải dữ liệu kiểm duyệt.'))
  ...
  const counts = {
    pending: items.filter((x) =>
      ['PENDING', 'PENDING_REVIEW'].includes(x.status || x.moderationStatus),
    ).length,
    active: items.filter((x) =>
      ['IN_REVIEW', 'RESTRICTED', 'APPEALED'].includes(x.status || x.moderationStatus),
    ).length,
    done: items.filter((x) =>
      ['APPROVED', 'NO_VIOLATION', 'VIOLATION_CONFIRMED', 'RESTORED', 'CLOSED'].includes(
        x.status || x.moderationStatus || x.newStatus,
      ),
    ).length,
  };
